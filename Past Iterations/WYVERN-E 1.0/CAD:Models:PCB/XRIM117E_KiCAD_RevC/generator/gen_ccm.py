...
OUT = os.path.join(os.path.dirname(__file__), '..', 'out', 'CCM_KiCAD_RevC')
os.makedirs(OUT, exist_ok=True)

# ───────── component & net tables ─────────
# RP2040 QFN-56 pin map (RP2040 datasheet §1.2, verified)
RP = {  # pin -> net
 1:"+3V3", 2:"UART0_TX", 3:"UART0_RX", 4:"CAM_D0", 5:"CAM_D1", 6:"CAM_D2",
 7:"CAM_D3", 8:"CAM_D4", 9:"CAM_D5", 10:"+3V3",
 11:"CAM_D6", 12:"CAM_D7", 13:"SPI_SCK", 14:"SPI_MOSI", 15:"SPI_MISO",
 ...
 31:"I2C0_SDA", 32:"I2C0_SCL", 33:"+3V3", 34:"PYRO2_GATE_MCU", 35:"CAM_XCLK",
 36:"CAM_PCLK", 37:"IMU_CS", 38:"PYRO1_CONT", 39:"PYRO2_CONT", 40:"CAM_HREF",
...
IMU = {1:"SPI_MISO",2:"",3:"",4:"",5:"+3V3",6:"GND",7:"GND",8:"+3V3",
       9:"GND",10:"",11:"",12:"IMU_CS",13:"SPI_SCK",14:"SPI_MOSI"}
...
BMP = {1:"+3V3",2:"I2C0_SCL",3:"GND",4:"I2C0_SDA",5:"GND",6:"+3V3",7:"",
       8:"GND",9:"GND",10:"+3V3"}
...
E22 = {1:"GND",2:"GND",3:"GND",4:"GND",5:"GND",6:"LORA_RXEN",7:"LORA_TXEN",
       ...
       14:"LORA_BUSY",15:"LORA_NRST",16:"SPI_MISO",17:"SPI_MOSI",18:"SPI_SCK",
...
def main():
    sch = Schematic("XRIM-117E CCM - Central Command Module (PDR-003 Rev C, 127mm finless airframe)",
        "Skylight Industries LLC / Legacy Systems Research Group",
        ["PDR-003 Rev C2 | + OV7670 camera (D0-D7=GPIO2-9 PIO, XCLK=GPIO23, SCCB on I2C0)",
         "2x pyro (drogue/main; CH3 traded for camera) | IMU+LoRa share SPI bus | 1S+USB",
         "All IC pinouts datasheet-verified. RF: keep antenna trace <4mm, pour-fenced."])
    brd = Board("XRIM-117E CCM Rev C", radius=50.0)
    ...
    S.place("XRIM:ICM42688","ICM42688","U3","ICM-42688-P",250,60,{str(k):v for k,v in IMU.items()},
            nc_pins=("2","3","4","10","11"))
    S.place("XRIM:BMP388","BMP388","U4","BMP388",310,60,{str(k):v for k,v in BMP.items()},nc_pins=("7",))
    ...
    # camera connector symbol (2x9)
    CAMP=["3V3","GND","SIOC","SIOD","VSYNC","HREF","PCLK","XCLK","D7","D6","D5","D4","D3","D2","D1","D0","RESET","PWDN"]
    CL=[(CAMP[i],str(i+1),"pas") for i in range(9)]
    CR=[(CAMP[i],str(i+1),"pas") for i in range(9,18)]
    sch.add_lib_symbol(make_box_symbol("XRIM","CAM18","J","OV7670","XRIM:PinSocket_2x09_P2.54mm",CL,CR,w=14),
                       "CAM18", Schematic.box_pins(CL,CR,w=14))
    CAMN={"1":"+3V3","2":"GND","3":"I2C0_SCL","4":"I2C0_SDA","5":"CAM_VSYNC","6":"CAM_HREF",
          "7":"CAM_PCLK","8":"CAM_XCLK","9":"CAM_D7","10":"CAM_D6","11":"CAM_D5","12":"CAM_D4",
          "13":"CAM_D3","14":"CAM_D2","15":"CAM_D1","16":"CAM_D0","17":"CAM_RST","18":"CAM_PWDN"}
    S.text("CAMERA: OV7670 2x9 header - SCCB on I2C0, D0-D7 -> GPIO2-9 (PIO), XCLK=GPIO23 PWM",220,370)
    S.place("XRIM:CAM18","CAM18","J11","OV7670",260,395,CAMN)
    S.place("XRIM:R","R","R30","10k",310,385,{"1":"+3V3","2":"CAM_RST"})
    S.place("XRIM:R","R","R31","10k",310,397,{"1":"CAM_PWDN","2":"GND"})
    # ── PCB ──
    F = parts
    ...
    m3=F.mount_m3()
    # 2x9 socket for OV7670 (faces board edge; camera pod on +X side of airframe)
    cam_pads=[]
    for i in range(9):
        for row in range(2):
            cam_pads.append(Pad(i*2+row+1, -10.16+2.54*i, -1.27+2.54*row, 1.7,1.7,
                shape="rect" if i==0 and row==0 else "circle", kind="thru_hole", drill=1.0))
    cam18=FP("PinSocket_2x09_P2.54mm",cam_pads,(-11.5,-2.6,11.5,2.6),
             ['(fp_rect (start -11.43 -2.54) (end 11.43 2.54) (stroke (width 0.12) (type default)) (fill none) (layer "F.SilkS") (uuid "TSTAMP"))'],
             attr="through_hole")
    # SMA diode footprint
    ...
    A(e22,"U5","E22-900M22S",98,64,0,sn(E22))
    ...
    A(xt,"J4","XT30",100,143.0,180,{"1":"VBAT","2":"GND"})
    A(sc2,"J5","ARM",61.2,120.7,118,{"1":"VBAT","2":"VBAT_ARMED"})
    A(sc2,"J6","PYRO1",77.3,137.7,149,{"1":"VBAT_ARMED","2":"PYRO1_D"})
    A(sc2,"J7","PYRO2",122.7,137.7,211,{"1":"VBAT_ARMED","2":"PYRO2_D"})
    A(sma,"J3","SMA",106.3,52.6,0,{"1":"RF_ANT","2":"GND","3":"GND","4":"GND","5":"GND"})
    A(gh8,"J1","SDM",57.0,100,90,{"1":"GND","2":"UART0_TX","3":"UART0_RX","4":"ARM_OUT","7":"GND","8":"GND","MP1":"GND","MP2":"GND"})
    A(sh4,"J9","USB",79.5,87.0,240,{"1":"USB5V","2":"USB_DM","3":"USB_DP","4":"GND","MP1":"GND","MP2":"GND"})
    A(cam18,"J11","OV7670",129.0,113.5,300,
      {"1":"+3V3","2":"GND","3":"I2C0_SCL","4":"I2C0_SDA","5":"CAM_VSYNC","6":"CAM_HREF",
       "7":"CAM_PCLK","8":"CAM_XCLK","9":"CAM_D7","10":"CAM_D6","11":"CAM_D5","12":"CAM_D4",
       "13":"CAM_D3","14":"CAM_D2","15":"CAM_D1","16":"CAM_D0","17":"CAM_RST","18":"CAM_PWDN"})
    A(r06,"R30","10k",121.0,107.0,30,{"1":"+3V3","2":"CAM_RST"})
    A(r06,"R31","10k",123.4,109.0,30,{"1":"CAM_PWDN","2":"GND"})
    ...
    CC=[("C1","10uF",90.5,110.5,90,{"1":"VIN_SYS","2":"GND"}),
        ...
        ("C22","100nF",88.0,64.0,90,{"1":"+3V3","2":"GND"}),
        ("C23","10uF",88.0,66.8,90,{"1":"+3V3","2":"GND"}),
    ...
    for i,(mx,my) in enumerate([(81.7,81.7),(118.3,81.7),(81.7,118.3),(118.3,118.3)]):
        ...
        sc=44.0/math.hypot(ddx,ddy)
        A(m3,f"H{i+1}","M3",100+ddx*sc,100+ddy*sc,0,{})

    # RF: manual route, fat & short
    antx, anty = 104.985, 64-8.509+1.27  # E22 pin21 abs
    brd.track("RF_ANT",[(antx,anty),(106.3,anty),(106.3,52.6)],"F.Cu",0.9)
    rf_keepout = sbox(104.2, 50.8, 108.4, anty+1.3)
    ...
    stitch=stitch_gnd(brd, spacing=7.0, avoid=[rf_keepout])
