...
- Recovery was previously debated between a timer-forced electronic deploy (finless-era) and an
  ...
  the **F15-4 motor's own ejection charge**, fired 4 s after burnout (t = 7.45 s, 0.78 s past
...
- This **eliminates** the RRC3+, the isolated 9 V recovery battery, the e-match/black-powder charge,
  and the earlier CO2 solenoid system entirely. The finned airframe (4×87 mm, +1.20 cal, 729 g
...
| Launch detect | \|a\| > **2 g** sustained ≥ 50 ms, body BNO085 accelerometer report (lowered from 3 g when liftoff mass went 705→792 g: peak specific force at 792 g was 3.27 g and the 3 g window was only 61 ms; the 2026-08-10 material re-zoning to 729 g raises peak specific force further, to 3.54 g, which only widens the margin the 2 g threshold already has) | flowcharts/01_flight_state_machine.mermaid, launch_status.h |
...
**Action items this reversal reopens:**
- ~~`Simulations/CFD/` was deleted from the repo during the 2026-08 removal~~, **restored
  2026-08-10** from git history (the pre-deletion commit, not a rebuild-from-scratch): vortex-panel
  solver, airfoil profile library, `airfoil_polars.csv`, `WYVERN_E2_airfoil_polars.xlsx`,
  `cl_alpha.png`, `cp_distribution.png` are all back and re-verified (solver re-run, validation line
  reproduces the documented NACA0012 dCl/dα ≈ 109.6% of thin-airfoil ideal; xlsx recalculated
  clean, zero formula errors, data matches the fresh CSV). RQ numbering in that folder's README and
  script updated from the old "RQ1" label to current RQ3 (fin aerofoil)/RQ4 (tunnel-vs-flight).
...
## 8. Custom PCB revision, 2026-08-11 fab set, board is now Ø61 mm circular

The `PCB/` folder's fab package (`Gerber_PCB1_2026-08-11.zip`, `SCH_Schematic1_2026-08-11.pdf`,
`BOM_Board1_PCB1_2026-08-11.xlsx`, `PickAndPlace_PCB1_2026_08_11.xlsx`, `Netlist_PCB1_2026-08-11.tel`,
Altium/PADS exports, 3D render) supersedes `PCB/Archive 2.zip` (the 2026-08-09 set). **Archive 2.zip
is still sitting in the live `PCB/` folder alongside the new files, not yet moved out**, worth
relocating to `Past Iterations/` so the two revisions don't get confused on the bench. Per
`PCB/Info_PCB_PCB1_2026-08-11.txt`, board size is **61 mm x 61 mm** (a circular Edge.Cuts outline,
confirmed against the render, so read that as **Ø61 mm**), up from whatever the 08-09 rev measured
(never documented numerically anywhere in this repo, so there's no prior figure being corrected
here, just the first time a board diameter has been written down).

**Fit check against the Upper BT (70 mm OD airframe, 1.6 mm wall per `WYVERN_E4_FEA_Structural.md`
§4 → ~66.8 mm ID):** Ø61 mm leaves **~5.8 mm diametral clearance (~2.9 mm radial per side)**. Fits,
but it's a tighter margin than a rough guess would suggest, worth carrying into the build guide's
Upper BT fit-check step rather than assuming it's obviously fine.

No other project doc (recovery, BOM, compatibility audit, mathematics) previously stated a numeric
PCB diameter, so this section is the new source of truth for board size going forward. If the
Ø61 mm figure changes again, update this entry and `WYVERN_E4_Recovery.md` §1 together.

## 9. CAD generator (`3D parts/_generator/gen_rocket4.py`), full rewrite 2026-08-11, was two full architecture revisions stale

`gen_rocket4.py` and every exported STEP/STL under `3D parts/` were still modeling the **old
single-tube, bypass-tube, dual-sealed-bulkhead** airframe with **PLA/PETG-CF** material zoning and
**72 mm** fins, none of which match current spec. Rewritten from scratch to match
`WYVERN_E4_Recovery.md` and the 2026-08-10 re-zoning:

- **Architecture:** Upper BT (ASA-Aero, nose + recovery wadding/FC bay) + Lower BT (PETG-CF, chute/
  wadding + TVC bay, one continuous tube) + **one** bulkhead joint (PETG-CF, wiring pass-throughs
  only, not gas-sealed). The bypass tube and second bulkhead are gone.
- **Materials:** ASA-Aero 0.65 g/cm³ (upper body), PETG-CF 1.30 g/cm³ (lower body + fins), PC-FR
  1.20 g/cm³ (motor mount + gimbal only), replacing the retired PLA/PETG-CF split.
- **Fins:** 87 mm span (was 72 mm), root 70 / tip 35 / LE-sweep 25 / thickness 3, matching
  `WYVERN_E4_Stability_FinSizing.md`.
- **Wall:** uniform 1.6 mm across all materials (the old PLA-only 1.2 mm thin-wall variant is gone
  along with PLA).

**Open items this rewrite does NOT resolve, flagged rather than guessed:**
1. **Upper BT / Lower BT tube lengths are SCALED-NOT-VERIFIED.** No doc states the length split
   directly (only masses, and `WYVERN_E4_Mathematics.md` §1 itself flags those as stale pending a
   sim re-run). The generator scales the old segment lengths to hit `core.py`'s LTOT=740 mm total,
   giving Upper BT ≈198 mm / Lower BT ≈422 mm. Treat as a first-pass placeholder for fit/mass
   checks, not a dimensioned-drawing source, until a real bay-layout pass replaces it.
2. **Bulkhead joint release geometry is not modeled.** The friction-fit/shear-pin sizing for the
   50–150 N release target (`WYVERN_E4_Recovery.md` §4, `WYVERN_E4_FEA_Structural.md` §4.1) is an
   open engineering decision per that doc, not invented here; the CAD bulkhead is a plain disk with
   two wiring pass-through holes only.
3. **Rail-button material is an assumption** (PETG-CF, matching the lower body it mounts to); no
   doc specifies it.
4. **Old files** (`01_nose_cone_ellipsoid_PLA`, `02_recovery_bay_PLA`, `03_fc_bay_PLA`,
   ...
   `09_bypass_tube_PETGCF`) **were removed 2026-08-11** once file-delete permission was granted;
   `3D parts/` now holds only the current-architecture parts.

Re-run with `cd "3D parts/_generator" && python3 gen_rocket4.py` after any further geometry change,
per the project's generate-then-render convention, don't hand-edit the STEP/STL.

## 10. Print-readiness (DFM) pass, 2026-08-11: rocket + servo TVC stand + static stand

**Scope:** every part in `3D parts/` (rocket), `Motor Test Stand/TVC Thrust-Vector Balance/`
(servo test stand), and `Motor Test Stand/Static Stand (thrust + deflector)/` (static stand).
**Explicitly out of scope:** the magnetic TVC stand (`3D parts/MTVC/`) and the wind tunnel
(`Wind Tunnel/`), neither touched by this pass.

Before this pass every part was a proof-of-concept boolean shape: mating surfaces were often
modeled at *exactly* the same radius (which does not go together in real life), there were zero
fastener holes or heat-set-insert bosses anywhere, and the gimbal's pivot "joints" were solid
bosses fused onto both halves of what's supposed to be a rotating assembly, un-printable as a
working hinge. `gen_rocket4.py` and `wcad.py` were extended with named DFM primitives
(`insert_boss`, `hole_cutter`, `bolt_circle`) and every part now carries:

- **Explicit fit tolerances**, named once (`FIT_SLIP=0.20mm` radial clearance for sliding/insert
  fits, `FIT_PRESS=-0.10mm` radial interference for friction-hold fits like centering rings on
  the motor tube), not accidental zero-clearance geometry.
- **M3 heat-set insert bosses** (4.2mm pilot, 8mm boss OD, standard knurled-brass-insert specs,
  catalog-standard and safe to trust) at every bolted joint: TVC balance base/thrust-block
  corners, static-stand base plate, HX711 mounts.
- **Retention screws** (M3 clearance, radial) holding the bulkhead captive in the Upper BT and
  the motor mount's aft centering ring captive in the Lower BT, replacing pure friction-fit with
  no mechanical backup.
- **Real through-holes for the gimbal's pivot pins** (3.2mm, slip fit for a 3mm steel dowel or M3
  shoulder screw) instead of the old solid dummy pins.
- **Three new static-stand parts** (`TS_base_plate`, `TS_motor_tower`, `TS_loadcell_bracket`)
  written from scratch: these existed as orphaned files with no generator (their old README
  credited `gen_rocket4.py`, which didn't contain them) and used a stale instrumentation spec
  (Wishiot 10kg cell + Metro M4) that never matched `WYVERN_E4_GSE_TestStands.md`'s 5kg cell +
  HX711 + RP2350B PCB chain. Rebuilt against the correct spec; the Static Stand README is fixed
  to match.
- **One new servo-stand part**, `TVC_balance_hx711_mount`.

**What this pass does NOT resolve, flagged rather than guessed, same standard as §8/§9:**
1. **Hardware interface dimensions are TYPICAL-CLASS, not measured.** Servo mounting-flange span
   (EMAX ES08MA II), load-cell body/hole spacing (Adafruit 5kg/1kg strain-gauge cells), and the
   HX711 breakout's hole pattern are catalog-typical figures for those specific parts, not
   calipers-verified against the bench hardware. Every such block in `gen_rocket4.py` is tagged
   `VERIFY-WITH-CALIPERS`. Treat as "will assemble," confirm before the final print run.
2. **Bulkhead release geometry still isn't modeled** (carried over from §9): the friction-fit/
   shear-pin sizing for the 50-150N release target is an open engineering decision, not invented
   here. This pass only adds retention screws to keep the bulkhead from rattling loose in
   *handling*, it does not resolve how the two body tubes release from *each other* at ejection.
3. **The gimbal's pitch-axis trunnion mount into the flight airframe is not modeled.** The servo
   test stand's `TVC_balance_thrust_block` DOES model a matching pivot mount (that geometry is
   pinned down for the bench rig), but the flight vehicle's equivalent (how the gimbal's outer
   ring actually anchors into the Lower BT wall) is the same class of open mechanism decision as
   item 2, not resolved by this pass.
4. **Print-orientation/support notes are not embedded in the CAD.** Wall thickness (1.6mm, FEA-
   verified) and the fin's flat-print/100%-infill callout (`WYVERN_E4_Build_Guide.md`) are the
   only orientation-relevant facts on record; a full per-part orientation/support pass (which
   face down, what needs support material) hasn't been done.

Re-run with `cd "3D parts/_generator" && python3 gen_rocket4.py` to regenerate after any further
change; it now also emits `TVC_balance_hx711_mount`, `TS_base_plate`, `TS_motor_tower`, and
`TS_loadcell_bracket` alongside the rocket parts.
