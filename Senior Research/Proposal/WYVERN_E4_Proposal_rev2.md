...
> houses avionics (nose, upper body tube), PETG-CF for the lower body and fins, PC-FR
...
>
> **2026-08-12 update:** a further bottom-up mass rebuild against the current CAD
> (`3D parts/_generator/mass_report.json`) and the real custom-PCB1 avionics stack — the
> Flight Computer Systems & Avionics and Power sections below have been rewritten to match, replacing
> the earlier, never-fabricated Pico 2 W/discrete-UBEC description — dropped liftoff to **698 g**
> (dry 638 g) by moving the Lower BT tube from PETG-CF to ASA-Aero (hoop-stress-checked at SF ≈6–10×
> under ejection pressure) and removing a discrete UBEC line that duplicated the onboard buck
> regulator. Every mass/CG/T-W/apogee figure below has been updated to match: CG 50.1 cm, CP 59.3 cm
> (unchanged), T/W 2.10/3.70 avg/peak, apogee ≈439 ft (133.7 m) at 6.87 s. See
...
**Table 1. GTR70E WYVERN mass budget** (2026-08-12 recompute; structural rows are real CAD output, not scaled estimates).
...
| Lower BT | **ASA-Aero** tube (changed from PETG-CF), **PC-FR** gimbal + motor mount | Chute + 1/8″ Kevlar shock cord, Nomex canopy protector, wadding, TVC bay (gimbal assembly, 2× servo actuators, external attitude IMU at the bulkhead boundary, motor mount) | 94.2 g tube + 57.7 g mount + 105.6 g gimbal + 70 g recovery |
...
The Lower BT tube's PETG-CF→ASA-Aero swap is hoop-stress-checked against the 140 kPa ejection pulse
(σ ≈ 2.93 MPa, SF ≈ 6–10×); the bulkhead (direct gas-exposure part) and the motor mount/gimbal
(thermal duty at the nozzle) keep their original materials. The custom PCB1 assembly mass is a
