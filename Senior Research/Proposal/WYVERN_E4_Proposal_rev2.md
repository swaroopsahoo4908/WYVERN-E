...
> ⚠ **Material re-zoning and canonical-figures update (2026-08-10).** The airframe material zoning
> changed from a two-way PLA/PETG-CF split to a three-way split: ASA-Aero for the upper body that
> houses avionics (nose, recovery bay tube, FC bay tube), PETG-CF for the lower body and fins, PC-FR
> for the TVC assembly (motor mount, gimbal). Holding geometry fixed and rescaling each item's mass
> by its new material density showed the lighter foamed-ASA upper body plus the heavier PETG-CF fins
> move CG aft enough, from 48.4 to roughly 50.4 cm against an unchanged 56.8 cm CP, to drop static
> margin under the 1.0 cal floor this program treats as the minimum for pre-TVC passive stability.
> Fin span was increased from 72 to 87 mm in the same pass to restore +1.20 cal. Every mass, CG, CP,
> T/W, and trajectory figure in this document has been updated to the resulting cascade-verified
> numbers: liftoff 729 g, dry 627 g, CG 50.8 cm, CP 59.3 cm, T/W 2.01/3.54 avg/peak, apogee ≈397 ft
> (121.1 m) at 6.67 s, independently re-derived from the component mass-moment stack in `we4_sim.py`
> and cross-checked against `we4_flightsim.py`'s RK4+Barrowman output and `WYVERN_E4_BUILD_READINESS.md`
> §9.4. This supersedes the prior 792 g/690 g/48.4 cm/324 ft figure set, which was itself a correction
> of an even older 705 g/603 g/435 ft set predating the i3 camera swap (see
> `WYVERN_E4_Camera_Solution.md`). The jetvane test in RQ2 was also redefined in this pass, from a
> mounted-vane erosion/deflection test to a flat-coupon blast-shield melt-through screen; see RQ2 and
> §5.4 below.
...
> **2026-08-10 scope restoration.** The five-research-question program below is the full, current
> scope. An interim revision of this proposal had dropped jetvane testing and removed the wind
> tunnel entirely, consolidating what were originally two separate aerofoil-related questions (fin
> selection, and tunnel-vs-flight calibration) into one. That consolidation is reversed as of
> 2026-08-10: RQ3 and RQ4 are un-merged below, and jetvane testing returns as part of RQ2.
...
|---|---|---|---|
| Nose cone | ASA-Aero | Ellipsoid nose | 16 g |
| Engine/TVC bay | **PETG-CF** tube + Bulkhead A, **PC-FR** gimbal + motor mount | Engine-bay tube, Bulkhead A, gimbal assembly, 2× servo actuators, attitude IMU, motor mount | 265 g |
| Flight-computer bay | ASA-Aero tube | FC-bay tube, Pico 2 W, attitude IMU, barometric sensors, microSD logger, 2S LiPo + 5 V UBEC, i3 4K Thumb Action Camera camera | 116 g |
| Recovery bay | ASA-Aero tube, **PETG-CF** gas-path hardware | Recovery-bay tube, parachute + 1/8″ Kevlar shock cord, Nomex canopy protector, redundant attitude IMU (vote), **PETG-CF** bypass ejection tube, **PETG-CF** sealed Bulkhead B, ejection plenum + nose retention | 137 g |
| Structure | PETG-CF | 4× fixed fins (87 mm), internal wiring | 93 g |
| **Dry mass total** | | | **627 g** |
| Motor (loaded) | — | Estes F15-4, 60 g propellant | 102 g |
| **Liftoff mass total** | | | **729 g** |

Foamed ASA-Aero forms the upper body (nose, flight-computer bay tube, recovery bay tube), the section that houses avionics and sees neither motor heat nor ejection gas; PETG-CF forms the lower body and fins, which carry the ejection-gas path and the fin aerodynamic load; and fire-retardant PC-FR forms the TVC assembly proper (motor mount and gimbal), the zone closest to sustained motor-plume heating. This three-way allocation reduces dry mass relative to an all-PETG-CF baseline (specified at 812 g liftoff in an earlier design iteration) while still meeting each zone's thermal and structural requirement, directly raising the thrust-to-weight ratio and predicted apogee. Because the lighter ASA-Aero upper body moves the center of gravity aft while the heavier PETG-CF fins do the same, the fin span was increased from 72 to 87 mm to preserve the 1.0-caliber static margin without nose ballast (see §4.2).
...
1.20 cal (87 mm fins, 729 g liftoff, ~397 ft) rather than the bare 1.0-cal minimum shown above.
...
> ⚠ **2026-08-10 restoration note.** The wind-tunnel campaign's original reference list (Hofferth,
> Bell & Mehta, Maskell, Mehta & Bradshaw, Pope & Harper, Selig, cited by author name only in
> `CONFLICTS.md` §7's removal record, without full bibliographic detail preserved anywhere in the
> repository) has not been reconstructed here, fabricating full citation entries from an author-name
> list alone risks getting the year, title, or venue wrong. Kuethe & Chow is added below since the
> restored CFD code (`Simulations/CFD/run_airfoil_cfd.py`) cites it directly as its solver
> methodology and the full reference is independently verifiable. Re-add the remaining wind-tunnel
> literature from the original source material before this proposal is treated as submission-final.
