...
| Upper BT | ASA-Aero tube | Nose, camera, flight computer (custom RP2350B PCB), body attitude IMU, barometric sensors, microSD logger, 2S LiPo + 5 V UBEC, recovery wadding | 116 g |
| Lower BT | **PETG-CF** tube, **PC-FR** gimbal + motor mount | Chute + 1/8″ Kevlar shock cord, Nomex canopy protector, recovery wadding, TVC bay (gimbal assembly, 2× servo actuators, external attitude IMU at the bulkhead boundary, motor mount) | 265 g |
| Bulkhead joint | PETG-CF | Single separation joint, wiring pass-throughs for servo extensions + STEMMA-QT cable | 137 g |
...
The vehicle's passive fin stability is retained through the coast phase to apogee (predicted apogee ≈ 397 ft at t ≈ 6.67 s). Recovery is initiated not by an independent electronic altimeter but by the flight motor's own factory ejection charge: the flight configuration uses an Estes F15-4 (a four-second ejection delay), so that approximately four seconds after propellant burnout, t ≈ 7.45 s, roughly 1.18 s past the predicted apogee, the motor's integral ejection charge fires inside the lower body tube's TVC bay. Gas pressure builds directly against the bulkhead joint between the two body tubes, and the joint — friction-fit/shear-pinned to release at a target force — separates, deploying the parachute packed at the forward end of the lower body tube.
