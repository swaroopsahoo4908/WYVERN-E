...
- `we4_flightsim.py` → `plots4/01_trajectory.png` + `flightsim_summary.json` — **unified RK4(2e-4) + Barrowman engine** (apogee 324 ft @ 6.27 s, motor-ejection deploy at t=7.45 s / 11.5 m/s, finned margin +1.20 cal).
...
Key results: liftoff 792 g · T/W 1.85/3.26 · CG 48.4 cm / CP 56.8 cm (+1.20 cal) · Iyy 0.0257 kg·m² ·
apogee 324 ft @ 6.27 s · v_max 29.7 m/s (Mach 0.087) · rail exit 11.5 m/s · min SF ~340× ·
...
792 g vehicle (rail-exit and weathercock margins per we4_stability.py). Mitigated by active
...
<1000 ft ceiling (324 ft / 98.9 m) and the long 2.95 s TVC window (the three-way trade is documented).
